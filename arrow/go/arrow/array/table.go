...
type Table interface {
	Schema() *arrow.Schema	// 字段列表
	NumRows() int64			// 行数
	NumCols() int64			// 列数
	Column(i int) *Column 	// 列类型、值
...
//
// 将多个原始的 array 组合成一个逻辑上的、大的 array 。
type Chunked struct {
	...
	chunks []Interface		// n 个 array

	length int				// 总元素数
	nulls  int				// 总空元素数
	dtype  arrow.DataType	// 数据类型
}
